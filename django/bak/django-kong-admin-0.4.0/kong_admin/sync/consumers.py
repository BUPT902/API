...
class ConsumerSyncEngine(KongProxySyncEngine):
    ...
    def before_withdraw(self, client, obj):
        ...
        for acls in AclReference.objects.filter(consumer=obj):
            self.acl().withdraw(client, acls)
...
class KeyAuthSyncEngine(ConsumerAuthSyncEngine):
    ...
    def on_publish(self, client, obj):
        ...
        """
        auth_struct = self.get_auth_client(client, obj.consumer.kong_id).create_or_update(
            key_auth_id=obj.kong_id, key=(obj.key or None))
        d = {}
        d['id'] = uuid.UUID(auth_struct['id'])
        d['key'] = uuid.UUID(auth_struct['key'])
        return d
...
class AclSyncEngine(ConsumerAuthSyncEngine):
    ...
    def on_publish(self, client, obj):
        ...
        """
        try:
            acl_struct = self.get_auth_client(client, obj.consumer.kong_id).create_or_update(
                acl_id=obj.kong_id, group=obj.group)
        except:
            return None
        return uuid.UUID(acl_struct['id'])
